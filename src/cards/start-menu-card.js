    ...
    return html`<ha-card class="start-menu-card">
      <div class="start-top">
        <div class="start-time-block">
          ...
          <time class="start-time" datetime=${now.toISOString()}>${new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit" }).format(now)}</time>
          ${this._config.show_date !== false
            ? html`<div class="start-date">${new Intl.DateTimeFormat(locale, { weekday: "long", day: "numeric", month: "long" }).format(now)}</div>`
            : nothing}
        </div>
